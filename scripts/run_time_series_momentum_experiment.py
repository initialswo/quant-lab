#!/usr/bin/env python3
"""Run a standalone time-series momentum sleeve experiment."""
...
RESULTS_ROOT = Path("results") / "time_series_momentum_experiment"
...
def _load_price_panels(args: argparse.Namespace) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, dict[str, Any], list[str], list[str]]:
    ...
    if not adj_cols or not close_cols:
        raise ValueError(
            "No usable price history found for time-series momentum experiment. "
            f"adj_missing={len(missing_adj)} adj_rejected={len(rejected_adj)} close_missing={len(missing_close)} close_rejected={len(rejected_close)}"
        )
    ...
    return close, adj_close, volume, data_summary, sorted(set(used_adj).intersection(set(used_close))), sorted(set(missing_adj).union(set(missing_close)))
...
def _build_weights(
    ...
    elig = eligibility.reindex(index=sig.index, columns=sig.columns).fillna(False).astype(bool)
    sig_lag = sig.shift(1)
    vol = adj_close.astype(float).pct_change().rolling(int(vol_lookback)).std(ddof=0).shift(1)
    rb = rebalance_mask(pd.DatetimeIndex(sig.index), str(rebalance))
    rb_dates = pd.DatetimeIndex(sig.index[rb])

    weights = pd.DataFrame(0.0, index=sig.index, columns=sig.columns, dtype=float)
    selected_counts = pd.Series(0, index=sig.index, dtype=int)
...
def main() -> None:
    ...
    strategy_name = f"time_series_momentum_{str(args.weighting).lower()}"
    print("TIME-SERIES MOMENTUM EXPERIMENT")
    print("-------------------------------")
    print(
        "Config: "
        f"universe={args.universe} universe_mode={DEFAULT_UNIVERSE_MODE} rebalance={args.rebalance} "
        f"weighting={args.weighting} costs_bps={float(args.costs_bps):.1f} lookback={int(args.lookback)} "
        "selection=all eligible assets with positive lagged time-series momentum signal"
    )
    ...
    signal = compute_factor(FACTOR_NAME, adj_close, lookback=int(args.lookback))
    weights_rebal, selected_counts, rb_dates = _build_weights(
        signal=signal,
    ...
    results_path = run_dir / "time_series_momentum_results.csv"
    ...
    manifest = {
        ...
        "script_name": "scripts/run_time_series_momentum_experiment.py",
        "factor_plugin_path": "src/quant_lab/factors/time_series_momentum.py",
        ...
        "config": {
            ...
            "vol_lookback": int(args.vol_lookback),
            "top_n": None,
            "selection_rule": "Hold all eligible assets with positive lagged 252-day sign signal.",
            "fallback_rule": "Zero exposure when no asset has positive signal.",
        },
        ...
        "notes": [
            "The canonical factor plugin lives under src/quant_lab/factors for registry discovery.",
            "A duplicate definition is also stored at factors/time_series_momentum.py to match the requested file path.",
            "This is a standalone sleeve experiment implemented at the script layer; engine code is unchanged.",
        ],
        "outputs": {
