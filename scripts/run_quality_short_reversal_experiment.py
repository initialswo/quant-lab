#!/usr/bin/env python3
"""Compare 1-month versus 5-day reversal inside the quality sleeve."""
...
RESULTS_ROOT = Path("results") / "quality_short_reversal_experiment"
...
STRATEGIES = [
    {
        ...
        "notes": "Canonical quality sleeve baseline with 1-month reversal.",
    },
    {
        "strategy_name": "reversal_5d_test",
        "factor_names": ["gross_profitability", "reversal_5d"],
        "factor_weights": [0.70, 0.30],
        "notes": "Replaces 1-month reversal with a 5-day pullback signal.",
    },
]
RESULT_COLUMNS = [
    "strategy_name",
...
def _print_summary(summary_df: pd.DataFrame) -> None:
    print("")
    print("QUALITY SHORT REVERSAL SUMMARY")
    print("------------------------------")
    print(
...
def main() -> None:
    ...
    print("QUALITY SHORT REVERSAL EXPERIMENT")
    print("---------------------------------")
    print(
        "Shared config: "
    ...
    print("- reversal_5d_test: gross_profitability=0.70, reversal_5d=0.30")
    ...
    results_path = run_dir / "quality_short_reversal_results.csv"
    summary_path = run_dir / "quality_short_reversal_summary.csv"
    ...
    test_row = results_df.loc[results_df["strategy_name"] == "reversal_5d_test"].iloc[0]
    manifest = {
        "timestamp": timestamp,
        "script_name": "scripts/run_quality_short_reversal_experiment.py",
        "start": str(args.start),
        "end": str(args.end),
        "universe": DEFAULT_UNIVERSE,
        "rebalance": DEFAULT_REBALANCE,
        ...
        "reversal_5d_definition": "-close.pct_change(5, fill_method=None)",
        "comparison": {
        ...
        "notes": [
            "This experiment changes only the reversal horizon inside the quality sleeve.",
            "reversal_5d uses only past prices through a 5-day percentage change and does not use forward-looking data.",
