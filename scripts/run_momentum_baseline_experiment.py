#!/usr/bin/env python3
"""Run the baseline momentum sleeve experiment."""
...
RESULTS_ROOT = Path("results") / "momentum_baseline_experiment"
...
STRATEGY = {
    ...
    "factor_weights": [1.0],
    "notes": "Baseline momentum sleeve using the existing momentum_12_1 factor.",
}
...
def main() -> None:
    ...
    print("MOMENTUM BASELINE EXPERIMENT")
    print("----------------------------")
    print(
        "Config: "
        ...
        f"factor_aggregation_method={FACTOR_AGGREGATION_METHOD} factor=momentum_12_1"
    )
    ...
    results_path = run_dir / "momentum_baseline_results.csv"
    summary_path = run_dir / "momentum_baseline_summary.csv"
    ...
    manifest = {
        "timestamp": timestamp,
        "script_name": "scripts/run_momentum_baseline_experiment.py",
        "start": str(args.start),
        "end": str(args.end),
        "universe": DEFAULT_UNIVERSE,
        "universe_mode": "dynamic",
        ...
        "notes": [
            "This baseline momentum sleeve uses the existing momentum_12_1 factor.",
            "The requested name momentum_12m does not exist in the repo; this script reuses the existing 12-1 momentum factor instead.",
        ],
    ...
    print("")
    print(results_df.to_string(index=False, formatters={col: _format_float for col in ["CAGR", "Vol", "Sharpe", "MaxDD", "Turnover"]}))
    print("")
