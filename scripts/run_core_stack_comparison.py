"""Head-to-head comparison: Core stack vs Core stack plus liquidity filter."""
...
def _full_row(variant: str, summary: dict) -> dict:
    return {
        ...
        "RegimeFilter": bool(summary.get("RegimeFilter", True)),
        ...
        "MaxLeverage": float(summary.get("MaxLeverage", 1.5)),
        "MinPrice": float(summary.get("MinPrice", float("nan"))),
        "MinAvgDollarVolume": float(summary.get("MinAvgDollarVolume", float("nan"))),
        "LiquidityLookback": int(summary.get("LiquidityLookback", 20)),
...
def main() -> None:
    ...
    for variant_cfg in VARIANTS:
        ...
        for start, end in SUBPERIODS:
            ...
            subperiod_rows.append(_subperiod_row(variant=variant, start=start, end=end, summary=part))
    ...
    for c in ["CAGR", "Vol", "Sharpe", "MaxDD", "LevAvg", "LevMax", "BullPct", "BearPct", "LiqEligibleMed"]:
    ...
    print("CORE STACK FULL-PERIOD COMPARISON")
    print("---------------------------------")
    print(disp_full.to_string(index=False))
    print("")
    print("CORE STACK SUBPERIOD STABILITY")
    print("------------------------------")
    ...
    out_csv = outdir / f"core_stack_comparison_{ts}.csv"
    out_json = outdir / f"core_stack_comparison_{ts}.json"
    out_sub_csv = outdir / f"core_stack_subperiods_{ts}.csv"
