#!/usr/bin/env python3
"""Run a post-fix breadth sweep for the canonical Quant Lab baseline strategy."""
...
RESULTS_ROOT = Path("results") / "baseline_breadth_sweep"
...
FACTOR_WEIGHTS = [0.70, 0.30, 0.00]
FACTOR_AGGREGATION_METHOD = "linear"
TOPN_VALUES = [25, 50, 75, 100, 150, 200, 300, 400]
DATA_SOURCE = "parquet"
...
def _print_summary(summary_df: pd.DataFrame, best_idx: int) -> None:
    print("")
    print("BASELINE BREADTH SUMMARY")
    print("------------------------")
...
def main() -> None:
    ...
    print("POST-FIX BASELINE BREADTH SWEEP")
    print("-------------------------------")
    print(
        "Config: "
    ...
    results_path = run_dir / "baseline_breadth_sweep_results.csv"
    summary_path = run_dir / "baseline_breadth_sweep_summary.csv"
    ...
    manifest = {
        "timestamp": timestamp,
        "script_name": "scripts/run_baseline_breadth_sweep.py",
        "start": str(args.start),
        "end": str(args.end),
        "universe": DEFAULT_UNIVERSE,
        "rebalance": DEFAULT_REBALANCE,
        ...
        "notes": [
            "This is the post-fix breadth sweep for the canonical baseline strategy.",
            "It uses the corrected runner-level adjusted-price PnL path and warehouse-backed legacy research store.",
            "Only Top N varies across runs; factor set, weighting, and linear aggregation remain fixed.",
        ],
        "runs": run_manifest,
    }
    ...
    _copy_latest(
        files={
            "baseline_breadth_sweep_results.csv": results_path,
            "baseline_breadth_sweep_summary.csv": summary_path,
