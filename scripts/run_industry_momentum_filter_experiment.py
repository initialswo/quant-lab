#!/usr/bin/env python3
"""Run the post-fix industry momentum filter test against the canonical baseline strategy."""
...
RESULTS_ROOT = Path("results") / "industry_momentum_filter_experiment"
...
INDUSTRY_MOMENTUM_FACTOR = "momentum_12_1"
MIN_PRICE = 5.0
...
def _compute_factor_panels(
    ...
    factor_names = FACTOR_NAMES + [INDUSTRY_MOMENTUM_FACTOR]
    ...
    momentum_raw = raw_scores[INDUSTRY_MOMENTUM_FACTOR].astype(float)
    return raw_scores, composite_scores, momentum_raw



def _build_industry_filter(
    ...
    allowed = pd.DataFrame(False, index=momentum_raw.index, columns=momentum_raw.columns)
    selected_counts = pd.Series(np.nan, index=momentum_raw.index, dtype=float)
    valid_industry = industry_by_ticker.notna() & industry_by_ticker.astype(str).str.strip().ne("")
    valid_tickers = [t for t in momentum_raw.columns if bool(valid_industry.get(t, False))]
    industry_lookup = industry_by_ticker.loc[valid_tickers].astype(str)

    for dt in momentum_raw.index[rb_mask]:
        row = momentum_raw.loc[dt, valid_tickers]
        ...
        industry_returns = valid.groupby(industries).mean().sort_values(ascending=False, kind="mergesort")
        top_industries = industry_returns.index[: int(industry_limit)]
...
def _simulate_strategy(
    ...
    if industry_limit is not None:
        industry_filter, industry_counts = _build_industry_filter(
...
def _print_summary(summary_df: pd.DataFrame) -> None:
    print("")
    print("INDUSTRY MOMENTUM FILTER SUMMARY")
    print("-------------------------------")
    print(
...
def main() -> None:
    ...
    print("POST-FIX INDUSTRY MOMENTUM FILTER EXPERIMENT")
    print("-------------------------------------------")
    print(
        "Config: "
    ...
    print(
        "Note: industry leadership is ranked each rebalance using the mean raw momentum_12_1 signal "
        "within mapped industry_ff48 groups."
    )
    ...
    _, composite_scores, momentum_raw = _compute_factor_panels(
        close=close,
        adj_close=adj_close,
    ...
    results_path = run_dir / "industry_momentum_filter_results.csv"
    summary_path = run_dir / "industry_momentum_filter_summary.csv"
    ...
    manifest = {
        "timestamp": timestamp,
        "script_name": "scripts/run_industry_momentum_filter_experiment.py",
        ...
        "notes": [
            "This is the post-fix industry momentum filter experiment against the canonical corrected baseline.",
            "Factors and portfolio PnL use adjusted prices consistently with the corrected runtime.",
            "Industry momentum is defined as the mean raw momentum_12_1 signal across mapped names in each industry_ff48 group.",
        ],
    ...
    if best_filtered_sharpe > baseline_sharpe:
        print("Industry filtering improves performance versus the unfiltered baseline.")
    else:
        print("Industry filtering does not improve performance versus the unfiltered baseline.")
