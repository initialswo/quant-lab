"""Diversification test: benchmark equity sleeve vs sector rotation best variant."""
...
def _find_sector_rotation_best_returns() -> Path:
    latest = Path("results/sector_rotation_benchmark/best_variant_returns.csv")
    if latest.exists():
        return latest
    cands = sorted(Path("results/sector_rotation_benchmark").glob("*/best_variant_returns.csv"))
    if not cands:
        raise FileNotFoundError("Could not locate sector rotation best_variant_returns.csv")
    return cands[-1]


def _drawdown(ret: pd.Series) -> pd.Series:
    equity = (1.0 + ret.astype(float).fillna(0.0)).cumprod()
    return equity / equity.cummax() - 1.0


def main() -> None:
    benchmark_path = _find_benchmark_v11_equity_curve()
    sector_path = _find_sector_rotation_best_returns()
    ...
    sector = (
        ...
        .rename(columns={"returns": "sector_rotation_return"})
        .set_index("date")
        .sort_index()
    )
    aligned = benchmark.join(sector, how="inner").dropna().copy()
    if aligned.empty:
        raise RuntimeError("No overlapping dates between benchmark and sector rotation returns.")

    corr = float(aligned["benchmark_return"].corr(aligned["sector_rotation_return"]))
    rolling_corr = aligned["benchmark_return"].rolling(252).corr(aligned["sector_rotation_return"])

    dd_bench = _drawdown(aligned["benchmark_return"])
    dd_sector = _drawdown(aligned["sector_rotation_return"])
    in_dd_bench = dd_bench < 0.0
    in_dd_sector = dd_sector < 0.0
    both = in_dd_bench & in_dd_sector
    either = in_dd_bench | in_dd_sector
    ...
    for name, (wb, ws) in blends.items():
        col = f"portfolio_{name.replace('%', 'pct').replace('/', '_').replace(' ', '').lower()}"
        out[col] = wb * out["benchmark_return"] + ws * out["sector_rotation_return"]
        m = compute_metrics(out[col])
        rows.append(
            {
                "Portfolio": name,
                "WeightBenchmark": wb,
                "WeightSectorRotation": ws,
                "CAGR": float(m["CAGR"]),
                "Vol": float(m["Vol"]),
        ...
        [
            {
                ...
                "sector_rotation_returns_path": str(sector_path),
            }
        ]
    )
    ...
    out_with_diag = out.assign(
        ...
        drawdown_sector_rotation=dd_sector,
        rolling_corr_252=rolling_corr,
        in_drawdown_benchmark=in_dd_bench.astype(int),
        in_drawdown_sector_rotation=in_dd_sector.astype(int),
        in_drawdown_both=both.astype(int),
    ).reset_index()

    ts = datetime.now(timezone.utc).strftime("%Y%m%d_%H%M%S")
    root = Path("results/sector_rotation_strategy_combination_test")
    outdir = root / ts
    ...
    payload_path = outdir / "summary.json"
    ...
    payload = {
        ...
        "artifacts": {
            "combined_returns": str(combined_path),
            "strategy_correlation": str(corr_path),
            "portfolio_summary": str(portfolio_path),
        },
    }
    ...
    print("SECTOR ROTATION COMBINATION TEST")
    print("-------------------------------")
