#!/usr/bin/env python3
"""Run final validation checks for the leading multi-horizon quality strategy."""
...
RESULTS_ROOT = Path("results") / "quality_multi_horizon_final_validation"
...
def _run_config(args: argparse.Namespace, scenario: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "factor_weights": list(FINAL_WEIGHTS),
...
def main() -> None:
    ...
    print("QUALITY MULTI-HORIZON FINAL VALIDATION")
    print("--------------------------------------")
    print(
        "Base config: "
    ...
    print(
        "Strategy weights: "
        f"gross_profitability={FINAL_WEIGHTS[0]:.2f}, reversal_1m={FINAL_WEIGHTS[1]:.2f}, reversal_5d={FINAL_WEIGHTS[2]:.2f}"
    )
    ...
    for scenario in SCENARIOS:
        ...
        print(
            ...
            f"costs_bps={float(scenario['costs_bps']):.1f}"
        )
    ...
    if not results_df.empty:
        ...
        results_df = results_df.sort_values(["Block", "Period/Scenario"], kind="mergesort").reset_index(drop=True)
    export_df = results_df.loc[:, RESULT_COLUMNS].copy() if not results_df.empty else pd.DataFrame(columns=RESULT_COLUMNS)

    results_path = run_dir / "quality_multi_horizon_final_validation_results.csv"
    summary_path = run_dir / "quality_multi_horizon_final_validation_summary.csv"
    ...
    manifest = {
        "timestamp": timestamp,
        "script_name": "scripts/run_quality_multi_horizon_final_validation.py",
        "base_config": {
            "universe": DEFAULT_UNIVERSE,
            "universe_mode": "dynamic",
            "rebalance": DEFAULT_REBALANCE,
        ...
        "strategy_weights": list(FINAL_WEIGHTS),
        "scenarios": [
            {
                "block": str(s["block"]),
    ...
    for block_name in ["Cost Sensitivity", "Subperiod Sanity"]:
