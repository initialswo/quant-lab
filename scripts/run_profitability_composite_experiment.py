#!/usr/bin/env python3
"""Run the post-fix profitability composite test against the canonical baseline."""
...
RESULTS_ROOT = Path("results") / "profitability_composite_experiment"
...
STRATEGIES = [
    ...
    {
        "strategy_name": "profitability_roa",
        "factor_names": ["gross_profitability", "roa", "reversal_1m"],
        "factor_weights": [0.40, 0.30, 0.30],
        "notes": "Adds ROA to the profitability sleeve.",
    },
    {
        "strategy_name": "profitability_composite",
        "factor_names": ["gross_profitability", "roa", "asset_turnover", "reversal_1m"],
        "factor_weights": [0.30, 0.30, 0.10, 0.30],
        "notes": "Adds ROA and asset_turnover to the profitability sleeve.",
    },
]
RESULT_COLUMNS = [
    "strategy_name",
...
def _print_summary(summary_df: pd.DataFrame) -> None:
    print("")
    print("PROFITABILITY COMPOSITE SUMMARY")
    print("-------------------------------")
    print(
        f"{'Strategy':24s} {'CAGR':>8s} {'Vol':>8s} {'Sharpe':>8s} {'MaxDD':>8s} {'Turnover':>10s}"
    )
    for _, row in summary_df.iterrows():
        print(
            f"{str(row['strategy_name']):24s} "
...
def main() -> None:
    ...
    print("POST-FIX PROFITABILITY COMPOSITE EXPERIMENT")
    print("-------------------------------------------")
    print(
        "Config: "
        f"universe={DEFAULT_UNIVERSE} rebalance={DEFAULT_REBALANCE} costs_bps={DEFAULT_COSTS_BPS} "
        f"top_n={DEFAULT_TOP_N} factor_aggregation_method={FACTOR_AGGREGATION_METHOD}"
    )
    print("Note: asset_turnover is defined as revenue / total_assets using PIT-aligned fundamentals.")
    ...
    results_path = run_dir / "profitability_composite_results.csv"
    summary_path = run_dir / "profitability_composite_summary.csv"
    ...
    manifest = {
        "timestamp": timestamp,
        "script_name": "scripts/run_profitability_composite_experiment.py",
        "start": str(args.start),
        "end": str(args.end),
        "universe": DEFAULT_UNIVERSE,
        "rebalance": DEFAULT_REBALANCE,
        ...
        "asset_turnover_definition": "revenue / total_assets using PIT-aligned fundamentals",
        ...
        "notes": [
            "This is the post-fix profitability composite test against the canonical corrected baseline.",
            "It uses the corrected runner-level adjusted-price PnL path and warehouse-backed legacy research store.",
            "asset_turnover is defined as revenue / total_assets using PIT-aligned fundamentals.",
        ],
    ...
    baseline_sharpe = float(summary_df.loc[summary_df["strategy_name"] == "baseline", "Sharpe"].iloc[0])
    best_name = str(best_row["strategy_name"])
    best_sharpe = float(best_row["Sharpe"])
    improved = bool(best_name != "baseline" and best_sharpe > baseline_sharpe)

    print("")
    print(
        f"Best configuration: {best_name} Sharpe={_format_float(best_sharpe)} CAGR={_format_float(float(best_row['CAGR']))} "
        f"Vol={_format_float(float(best_row['Vol']))} MaxDD={_format_float(float(best_row['MaxDD']))} "
        f"Turnover={_format_float(float(best_row['Turnover']))}"
    )
    if improved:
        print(
            f"Profitability composite improves baseline: best Sharpe exceeds baseline by {_format_float(best_sharpe - baseline_sharpe)}."
        )
    else:
        print(
            f"Profitability composite does not improve baseline: baseline Sharpe remains ahead by {_format_float(baseline_sharpe - best_sharpe)}."
        )
