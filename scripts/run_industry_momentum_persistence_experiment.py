#!/usr/bin/env python3
"""Run the post-fix industry momentum persistence test against the canonical baseline strategy."""
...
RESULTS_ROOT = Path("results") / "industry_momentum_persistence_experiment"
...
HORIZON_WINDOWS = {"3m": 63, "6m": 126, "12m": 252}
STRATEGIES = [
    ("baseline", None),
    ("top10_persistent", 10),
    ("top20_persistent", 20),
...
SUMMARY_COLUMNS = ["strategy_name", "industry_limit", "CAGR", "Vol", "Sharpe", "MaxDD", "Turnover"]
...
def _compute_baseline_scores(close: pd.DataFrame, adj_close: pd.DataFrame, fundamentals_path: str) -> pd.DataFrame:
...
def _compute_stock_momentum_horizons(adj_close: pd.DataFrame) -> dict[str, pd.DataFrame]:
    return {name: adj_close.pct_change(window).astype(float) for name, window in HORIZON_WINDOWS.items()}
...
def _build_persistence_filter(
    ...
    sample_panel = next(iter(horizon_returns.values()))
    allowed = pd.DataFrame(False, index=sample_panel.index, columns=sample_panel.columns)
    selected_counts = pd.Series(np.nan, index=sample_panel.index, dtype=float)
    valid_industry = industry_by_ticker.notna() & industry_by_ticker.astype(str).str.strip().ne("")
    valid_tickers = [t for t in sample_panel.columns if bool(valid_industry.get(t, False))]
    industry_lookup = industry_by_ticker.loc[valid_tickers].astype(str)

    for dt in sample_panel.index[rb_mask]:
        elig = eligibility.loc[dt, valid_tickers].astype(bool)
        if int(elig.sum()) == 0:
            continue
        industry_scores: dict[str, pd.Series] = {}
        union_tickers: set[str] = set()
        for horizon_name, panel in horizon_returns.items():
            row = panel.loc[dt, valid_tickers]
            valid = row.where(elig).dropna()
            if valid.empty:
                industry_scores[horizon_name] = pd.Series(dtype=float)
                continue
            industries = industry_lookup.reindex(valid.index)
            industry_scores[horizon_name] = valid.groupby(industries).mean().astype(float)
            union_tickers.update(valid.index.tolist())

        if not union_tickers:
            continue

        all_industries = pd.Index(sorted(set().union(*[s.index.tolist() for s in industry_scores.values() if not s.empty])))
        if all_industries.empty:
            continue

        persistence = pd.Series(0.0, index=all_industries, dtype=float)
        valid_horizons = 0
        for horizon_name in ["3m", "6m", "12m"]:
            industry_ret = industry_scores.get(horizon_name, pd.Series(dtype=float)).reindex(all_industries)
            valid_ind = industry_ret.dropna()
            if valid_ind.empty:
                continue
            z = _zscore_series(valid_ind)
            persistence.loc[z.index] = persistence.loc[z.index] + z
            valid_horizons += 1

        if valid_horizons == 0:
            continue

        ranked = persistence.sort_values(ascending=False, kind="mergesort")
        top_industries = ranked.index[: int(industry_limit)]
        eligible_tickers = elig.index[elig]
        industries = industry_lookup.reindex(eligible_tickers)
        mask = industries.isin(top_industries)
        allowed.loc[dt, eligible_tickers[mask]] = True
        selected_counts.loc[dt] = float(len(top_industries))
...
def _simulate_strategy(
    ...
    if industry_limit is not None:
        industry_filter, industry_counts = _build_persistence_filter(
            horizon_returns=horizon_returns,
            eligibility=eligibility,
            industry_by_ticker=industry_by_ticker,
            industry_limit=int(industry_limit),
            rb_mask=rb_mask,
        )
        filtered_eligibility = (
            eligibility.reindex(index=scores.index, columns=scores.columns).fillna(False).astype(bool)
            & industry_filter.reindex(index=scores.index, columns=scores.columns).fillna(False).astype(bool)
        )
        scores = apply_universe_filter_to_scores(scores, filtered_eligibility, exempt=set()).astype(float)
        avg_selected_industries = float(industry_counts.dropna().mean()) if industry_counts.notna().any() else float("nan")
    ...
    row = {
        ...
        "industry_limit": int(industry_limit) if industry_limit is not None else pd.NA,
...
def _print_summary(summary_df: pd.DataFrame) -> None:
    print("")
    print("INDUSTRY MOMENTUM PERSISTENCE SUMMARY")
    print("------------------------------------")
    print(
        f"{'Strategy':18s} {'Limit':>5s} {'CAGR':>8s} {'Vol':>8s} {'Sharpe':>8s} {'MaxDD':>8s} {'Turnover':>10s}"
    )
    for _, row in summary_df.iterrows():
        limit = "-" if pd.isna(row["industry_limit"]) else str(int(row["industry_limit"]))
        print(
...
def main() -> None:
    ...
    print("POST-FIX INDUSTRY MOMENTUM PERSISTENCE EXPERIMENT")
    print("-------------------------------------------------")
    print(
        "Config: "
    ...
    print(
        "Note: industry persistence is ranked each rebalance using cross-sectionally standardized "
        "industry mean adjusted returns over 3m, 6m, and 12m horizons."
    )
    ...
    composite_scores = _compute_baseline_scores(
        close=close,
        adj_close=adj_close,
        fundamentals_path=str(args.fundamentals_path),
    )
    horizon_returns = _compute_stock_momentum_horizons(adj_close=adj_close)

    rows: list[dict[str, Any]] = []
    run_manifest: list[dict[str, Any]] = []
    for strategy_name, industry_limit in STRATEGIES:
    ...
    filtered_df = results_df.loc[results_df["strategy_name"] != "baseline"].copy()
    best_filtered = filtered_df.sort_values(by=["Sharpe", "CAGR"], ascending=[False, False], kind="mergesort").iloc[0]
    best_filtered_sharpe = float(best_filtered["Sharpe"])

    manifest = {
        "timestamp": timestamp,
        "script_name": "scripts/run_industry_momentum_persistence_experiment.py",
        ...
        "fundamentals_path": str(args.fundamentals_path),
        "industry_persistence_definition": {
            "industry_classification": "industry_ff48",
            "price_panel": "adj_close",
            "stock_horizons": {k: int(v) for k, v in HORIZON_WINDOWS.items()},
            "industry_aggregation": "mean stock return within industry",
            "cross_sectional_standardization": "z-score by horizon across industries on each rebalance date",
            "persistence_score": "zscore(3m) + zscore(6m) + zscore(12m)",
        },
        ...
        "best_filtered_strategy": str(best_filtered["strategy_name"]),
        "best_filtered_sharpe": best_filtered_sharpe,
        "runtime_seconds": float(time.perf_counter() - t0),
        "notes": [
            "This is the post-fix industry momentum persistence experiment against the canonical corrected baseline.",
            "Factors and portfolio PnL use adjusted prices consistently with the corrected runtime.",
            "Industry persistence is defined from cross-sectionally standardized 3m, 6m, and 12m industry mean adjusted returns.",
        ],
    ...
    print(f"Best filtered Sharpe: {_format_float(best_filtered_sharpe)} ({best_filtered['strategy_name']})")
    if best_filtered_sharpe > baseline_sharpe:
        print("Persistence filtering improves performance versus the unfiltered baseline.")
    else:
        print("Persistence filtering does not improve performance versus the unfiltered baseline.")
