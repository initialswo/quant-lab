"""Sweep liquidity ADV thresholds on the fixed core stack architecture."""
...
VARIANTS: list[dict[str, object]] = [
    {"Variant": "ADV_0", "min_avg_dollar_volume": 0.0},
    {"Variant": "ADV_5M", "min_avg_dollar_volume": 5_000_000.0},
    {"Variant": "ADV_10M", "min_avg_dollar_volume": 10_000_000.0},
    {"Variant": "ADV_20M", "min_avg_dollar_volume": 20_000_000.0},
]
...
def _full_row(variant: str, summary: dict) -> dict:
    return {
        "Variant": variant,
        "MinAvgDollarVolume": float(summary.get("MinAvgDollarVolume", float("nan"))),
        "LiquidityEligibleMedian": float(summary.get("LiquidityEligibleMedian", float("nan"))),
        "LiquidityFilteredOutMedian": float(summary.get("LiquidityFilteredOutMedian", float("nan"))),
...
def main() -> None:
    ...
    disp_full = full_df[
        ["Variant", "MinAvgDollarVolume", "LiquidityEligibleMedian", "CAGR", "Vol", "Sharpe", "MaxDD"]
    ].copy()
    disp_full.columns = ["Variant", "ADV", "LiqEligibleMed", "CAGR", "Vol", "Sharpe", "MaxDD"]
    for c in ["ADV", "LiqEligibleMed", "CAGR", "Vol", "Sharpe", "MaxDD"]:
    ...
    print("LIQUIDITY THRESHOLD SWEEP — FULL PERIOD")
    print("---------------------------------------")
    print(disp_full.to_string(index=False))
    print("")
    print("LIQUIDITY THRESHOLD SWEEP — SUBPERIOD STABILITY")
    print("-----------------------------------------------")
    ...
    out_csv = outdir / f"liquidity_threshold_sweep_{ts}.csv"
    out_json = outdir / f"liquidity_threshold_sweep_{ts}.json"
    out_sub_csv = outdir / f"liquidity_threshold_subperiods_{ts}.csv"
