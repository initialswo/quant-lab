"""Diversification test: long-only benchmark vs Growth Leader sleeve."""
...
def _find_growth_leader_best_returns() -> Path:
    latest = Path("results/growth_leader_benchmark/best_variant_returns.csv")
    if latest.exists():
        return latest
    cands = sorted(Path("results/growth_leader_benchmark").glob("*/best_variant_returns.csv"))
    if not cands:
        raise FileNotFoundError("Could not locate growth_leader best_variant_returns.csv")
    return cands[-1]
...
def main() -> None:
    ...
    gl_path = _find_growth_leader_best_returns()
    ...
    growth = (
        pd.read_csv(gl_path, parse_dates=["date"])[["date", "returns"]]
        .rename(columns={"returns": "growth_leader_return"})
        .set_index("date")
        .sort_index()
    )
    aligned = benchmark.join(growth, how="inner").dropna().copy()
    if aligned.empty:
        raise RuntimeError("No overlapping dates between benchmark and growth-leader returns.")

    corr = float(aligned["benchmark_return"].corr(aligned["growth_leader_return"]))
    rolling_corr = aligned["benchmark_return"].rolling(252).corr(aligned["growth_leader_return"])

    dd_bench = _drawdown(aligned["benchmark_return"])
    dd_growth = _drawdown(aligned["growth_leader_return"])
    in_dd_bench = dd_bench < 0.0
    in_dd_growth = dd_growth < 0.0
    both = in_dd_bench & in_dd_growth
    either = in_dd_bench | in_dd_growth
    ...
    for name, (wb, wg) in blends.items():
        col = f"portfolio_{name.replace('%', 'pct').replace('/', '_').replace(' ', '').lower()}"
        out[col] = wb * out["benchmark_return"] + wg * out["growth_leader_return"]
        m = compute_metrics(out[col])
        rows.append(
            {
                "Portfolio": name,
                "WeightBenchmark": wb,
                "WeightGrowthLeader": wg,
                "CAGR": float(m["CAGR"]),
                "Vol": float(m["Vol"]),
        ...
        [
            {
                ...
                "growth_leader_returns_path": str(gl_path),
            }
        ]
    )
    ...
    out_with_diag = out.assign(
        ...
        drawdown_growth_leader=dd_growth,
        rolling_corr_252=rolling_corr,
        in_drawdown_benchmark=in_dd_bench.astype(int),
        in_drawdown_growth_leader=in_dd_growth.astype(int),
        in_drawdown_both=both.astype(int),
    ).reset_index()

    ts = datetime.now(timezone.utc).strftime("%Y%m%d_%H%M%S")
    root = Path("results/growth_leader_strategy_combination_test")
    outdir = root / ts
    ...
    print("GROWTH LEADER STRATEGY COMBINATION TEST")
    print("---------------------------------------")
