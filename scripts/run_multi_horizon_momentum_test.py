"""Compare current lead vs multi-horizon momentum stack."""
...
def main() -> None:
    ...
    variants = [
        ...
        },
        {
            ...
            # Split legacy momentum weight across 3/6/12 horizons; keep other sleeves unchanged.
            "factor_weights": [0.43 / 3.0, 0.43 / 3.0, 0.43 / 3.0, 0.27, 0.15, 0.15],
        },
    ]
    ...
    for variant in variants:
        ...
        cfg["factor_name"] = list(variant["factor_names"])  # back-compat field
        cfg["factor_names"] = list(variant["factor_names"])
        cfg["factor_weights"] = list(variant["factor_weights"])
    ...
    base_out = Path("results") / "multi_horizon_momentum_test"
    base_out.mkdir(parents=True, exist_ok=True)
    ts = datetime.utcnow().strftime("%Y%m%d_%H%M%S")
    csv_path = base_out / f"comparison_{ts}.csv"
    json_path = base_out / f"comparison_{ts}.json"
    latest_csv = base_out / "comparison_latest.csv"
    latest_json = base_out / "comparison_latest.json"
    ...
    print("MULTI-HORIZON MOMENTUM COMPARISON")
    print("---------------------------------")
    print(
        df[
            [
