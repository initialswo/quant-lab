"""Compare rev_tilt_2 against a classic value sleeve upgrade."""
...
RESULTS_ROOT = Path("results") / "rev_tilt_2_value_upgrade"
START = "2000-01-01"
END = "2024-12-31"
MAX_TICKERS = 300
...
UPGRADE_WEIGHTS: dict[str, float] = {
    "sleeve_reversal": 0.40,
    "sleeve_gross_profitability": 0.20,
    "sleeve_momentum": 0.20,
    "sleeve_low_vol": 0.10,
    "sleeve_book_to_market": 0.10,
}
...
def _run_sleeves(book_to_market_params: dict[str, Any]) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
...
def _build_manifest(
    ...
    return {
        ...
        "strategy_definitions": {
            ...
            "rev_tilt_2_value_upgrade": UPGRADE_WEIGHTS,
        },
        "notes": [
            "Portfolios are return-level combinations of single-factor sleeves.",
            "book_to_market is a classic value sleeve using PIT shareholders_equity / market_cap.",
            "All sleeve runs reuse run_backtest with the default causal execution behavior.",
            "PIT-aligned fundamentals are passed explicitly for the book_to_market sleeve.",
        ],
...
def _print_interpretation(metrics_df: pd.DataFrame) -> None:
    ...
    upgrade = by_name.loc["rev_tilt_2_value_upgrade"]
    sharpe_note = (
        f"Sharpe improved ({upgrade['Sharpe']:.4f} vs {original['Sharpe']:.4f})."
        if float(upgrade["Sharpe"]) > float(original["Sharpe"])
        else f"Sharpe did not improve ({upgrade['Sharpe']:.4f} vs {original['Sharpe']:.4f})."
    )
    dd_note = (
...
def main() -> None:
    ...
    sleeve_payloads, sleeve_manifest = _run_sleeves(book_to_market_params=book_to_market_params)
    ...
    composite._copy_latest(
        files={
            "value_upgrade_results.csv": results_path,
    ...
    print("REV_TILT_2 VALUE UPGRADE TEST")
    print("-----------------------------")
    print("")
