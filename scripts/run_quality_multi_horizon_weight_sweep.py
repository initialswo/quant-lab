#!/usr/bin/env python3
"""Sweep multi-horizon reversal weights inside the quality sleeve."""
...
RESULTS_ROOT = Path("results") / "quality_multi_horizon_weight_sweep"
...
VARIANTS = [
    {
        "strategy_name": "rev1m_025_rev5d_005",
        "factor_weights": [0.70, 0.25, 0.05],
    },
    {
        "strategy_name": "rev1m_020_rev5d_010",
        "factor_weights": [0.70, 0.20, 0.10],
    },
    {
        "strategy_name": "rev1m_015_rev5d_015",
        "factor_weights": [0.70, 0.15, 0.15],
    },
    {
...
def _print_summary(summary_df: pd.DataFrame) -> None:
    print("")
    print("QUALITY MULTI-HORIZON WEIGHT SWEEP")
    print("----------------------------------")
    print(
...
def main() -> None:
    ...
    results_path = run_dir / "quality_multi_horizon_weight_sweep_results.csv"
    summary_path = run_dir / "quality_multi_horizon_weight_sweep_summary.csv"
    ...
    manifest = {
        "timestamp": timestamp,
        "script_name": "scripts/run_quality_multi_horizon_weight_sweep.py",
        "start": str(args.start),
        "end": str(args.end),
        "universe": DEFAULT_UNIVERSE,
        "universe_mode": "dynamic",
        ...
        "notes": [
            "This sweep keeps gross_profitability fixed at 0.70 and tunes the split between reversal_1m and reversal_5d.",
