#!/usr/bin/env python3
"""Compare the canonical quality sleeve against a quality-plus-investment variant."""
...
RESULTS_ROOT = Path("results") / "quality_investment_experiment"
...
STRATEGIES = [
    ...
    {
        "strategy_name": "baseline_plus_asset_growth",
        "factor_names": ["gross_profitability", "reversal_1m", "asset_growth"],
        "factor_weights": [0.60, 0.25, 0.15],
        "notes": "Adds a modest investment sleeve while preserving profitability as the dominant weight.",
    },
]
RESULT_COLUMNS = [
    "strategy_name",
...
def _build_asset_growth_params(start: str, end: str, max_tickers: int, fundamentals_path: str) -> dict[str, Any]:
    close = _load_close_panel(start=start, end=end, max_tickers=max_tickers)
    fundamentals = load_fundamentals_file(path=str(fundamentals_path), fallback_lag_days=60)
...
def _run_config(args: argparse.Namespace, strategy: dict[str, Any], asset_growth_params: dict[str, Any]) -> dict[str, Any]:
    cfg = {
    ...
    if "asset_growth" in strategy["factor_names"]:
        cfg["factor_params"] = {"asset_growth": dict(asset_growth_params)}
    return cfg
...
def _print_summary(summary_df: pd.DataFrame) -> None:
    print("")
    print("QUALITY PLUS INVESTMENT SUMMARY")
    print("-------------------------------")
    print(
...
def main() -> None:
    ...
    print("QUALITY PLUS INVESTMENT EXPERIMENT")
    print("----------------------------------")
    print(
        "Shared config: "
    ...
    print("- baseline_plus_asset_growth: gross_profitability=0.60, reversal_1m=0.25, asset_growth=0.15")

    t0 = time.perf_counter()
    asset_growth_params = _build_asset_growth_params(
        start=str(args.start),
        end=str(args.end),
        max_tickers=int(args.max_tickers),
        fundamentals_path=str(args.fundamentals_path),
    ...
    for strategy in STRATEGIES:
        cfg = _run_config(args=args, strategy=strategy, asset_growth_params=asset_growth_params)
    ...
    results_path = run_dir / "quality_investment_results.csv"
    summary_path = run_dir / "quality_investment_summary.csv"
    ...
    test_row = results_df.loc[results_df["strategy_name"] == "baseline_plus_asset_growth"].iloc[0]
    manifest = {
        "timestamp": timestamp,
        "script_name": "scripts/run_quality_investment_experiment.py",
        "start": str(args.start),
        "end": str(args.end),
        "universe": DEFAULT_UNIVERSE,
        "rebalance": DEFAULT_REBALANCE,
        ...
        "asset_growth_definition": "score = -((total_assets_t / total_assets_t_252) - 1)",
        "comparison": {
        ...
        "notes": [
            "This experiment changes only the factor set and factor weights relative to the quality baseline.",
            "asset_growth reuses the existing PIT-aligned factor implementation and receives aligned total_assets through script-level factor_params.",
