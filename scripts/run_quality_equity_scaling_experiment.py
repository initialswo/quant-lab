#!/usr/bin/env python3
"""Compare asset-scaled versus equity-scaled profitability in the quality sleeve."""
...
RESULTS_ROOT = Path("results") / "quality_equity_scaling_experiment"
...
STRATEGIES = [
    {
        ...
        "notes": "Asset-scaled profitability baseline.",
    },
    {
        "strategy_name": "gross_profit_to_equity_test",
        "factor_names": ["gross_profit_to_equity", "reversal_1m"],
        "factor_weights": [0.70, 0.30],
        "notes": "Replaces asset-scaled profitability with equity-scaled profitability.",
    },
]
RESULT_COLUMNS = [
    "strategy_name",
...
def _build_gross_profit_to_equity_params(start: str, end: str, max_tickers: int, fundamentals_path: str) -> dict[str, Any]:
...
def _run_config(args: argparse.Namespace, strategy: dict[str, Any], gpte_params: dict[str, Any]) -> dict[str, Any]:
    cfg = {
    ...
    if "gross_profit_to_equity" in strategy["factor_names"]:
        cfg["factor_params"] = {"gross_profit_to_equity": dict(gpte_params)}
    return cfg
...
def _print_summary(summary_df: pd.DataFrame) -> None:
    print("")
    print("QUALITY EQUITY SCALING SUMMARY")
    print("------------------------------")
    print(
...
def main() -> None:
    ...
    print("QUALITY EQUITY SCALING EXPERIMENT")
    print("---------------------------------")
    print(
        "Shared config: "
    ...
    print("- gross_profit_to_equity_test: gross_profit_to_equity=0.70, reversal_1m=0.30")

    t0 = time.perf_counter()
    gpte_params = _build_gross_profit_to_equity_params(
        start=str(args.start),
        end=str(args.end),
        max_tickers=int(args.max_tickers),
        fundamentals_path=str(args.fundamentals_path),
    ...
    for strategy in STRATEGIES:
        cfg = _run_config(args=args, strategy=strategy, gpte_params=gpte_params)
    ...
    results_path = run_dir / "quality_equity_scaling_results.csv"
    summary_path = run_dir / "quality_equity_scaling_summary.csv"
    ...
    test_row = results_df.loc[results_df["strategy_name"] == "gross_profit_to_equity_test"].iloc[0]
    manifest = {
        "timestamp": timestamp,
        "script_name": "scripts/run_quality_equity_scaling_experiment.py",
        "start": str(args.start),
        "end": str(args.end),
        "universe": DEFAULT_UNIVERSE,
        "rebalance": DEFAULT_REBALANCE,
        ...
        "gross_profit_to_equity_definition": "gross_profit / shareholders_equity with non-positive equity masked to NaN",
        "comparison": {
        ...
        "notes": [
            "This experiment changes only the profitability definition inside the quality sleeve.",
            "gross_profit_to_equity reuses existing PIT gross_profit and shareholders_equity fields.",
