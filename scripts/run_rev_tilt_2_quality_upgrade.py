"""Compare rev_tilt_2 against a quality-momentum sleeve upgrade."""
...
import run_quality_momentum_sleeve as quality_momentum
from quant_lab.engine.runner import run_backtest
from quant_lab.research.sweep_metrics import extract_annual_turnover


RESULTS_ROOT = Path("results") / "rev_tilt_2_quality_upgrade"
START = "2000-01-01"
END = "2024-12-31"
MAX_TICKERS = 2000
...
SLEEVE_SPECS: list[dict[str, Any]] = [
    ...
    {
        "strategy_name": "sleeve_quality_momentum",
        "factor_name": "quality_momentum_score",
        "factor_params": {},
        "notes": "Combined quality-momentum sleeve.",
    },
    {
...
ORIGINAL_WEIGHTS: dict[str, float] = {
    ...
    "sleeve_low_vol": 0.10,
    "sleeve_gross_profitability": 0.35,
}
UPGRADE_WEIGHTS: dict[str, float] = {
    "sleeve_quality_momentum": 0.15,
    "sleeve_reversal": 0.40,
    "sleeve_low_vol": 0.10,
    "sleeve_gross_profitability": 0.35,
}
...
def _run_sleeves(quality_params: dict[str, Any]) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    for raw_spec in SLEEVE_SPECS:
        ...
        if strategy_name == "sleeve_quality_momentum":
...
def _combine_portfolios(sleeve_payloads: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    upgrade = composite.weighted_sleeve_portfolio(
        ...
        name="rev_tilt_2_quality_upgrade",
    )
...
def _build_manifest(
    ...
    return {
        ...
        "strategy_definitions": {
            ...
            "rev_tilt_2_quality_upgrade": UPGRADE_WEIGHTS,
        },
        "notes": [
            "Portfolios are return-level combinations of single-factor sleeves.",
            "quality_momentum_score is the average of cross-sectional momentum and profitability percentile ranks.",
...
def _print_interpretation(metrics_df: pd.DataFrame) -> None:
    ...
    upgrade = by_name.loc["rev_tilt_2_quality_upgrade"]
    sharpe_note = (
        f"Quality upgrade improved Sharpe ({upgrade['Sharpe']:.4f} vs {original['Sharpe']:.4f})."
        if float(upgrade["Sharpe"]) > float(original["Sharpe"])
        else f"Quality upgrade did not improve Sharpe ({upgrade['Sharpe']:.4f} vs {original['Sharpe']:.4f})."
    )
    dd_note = (
...
def main() -> None:
    ...
    quality_params = quality_momentum._build_quality_momentum_params(
        start=START,
        end=END,
        max_tickers=MAX_TICKERS,
    )
    sleeve_payloads, sleeve_manifest = _run_sleeves(quality_params=quality_params)
    ...
    composite._copy_latest(
        files={
            "quality_upgrade_results.csv": results_path,
    ...
    print("REV_TILT_2 QUALITY UPGRADE TEST")
    print("--------------------------------")
    print("")
