#!/usr/bin/env python3
"""Compare the baseline quality sleeve against a quality-plus-asset-growth variant."""
...
RESULTS_ROOT = Path("results") / "quality_asset_growth_experiment"
...
STRATEGIES: list[dict[str, Any]] = [
    ...
    {
        "strategy_name": "quality_with_asset_growth",
        "factor_names": ["gross_profitability", "asset_growth", "reversal_1m", "reversal_5d"],
        "factor_weights": [0.50, 0.20, 0.05, 0.25],
        "notes": "Adds asset_growth while preserving the existing reversal mix.",
    },
]
...
def main() -> None:
    ...
    print("QUALITY ASSET GROWTH EXPERIMENT")
    print("-------------------------------")
    print(
        "Shared config: "
    ...
    print("- quality_with_asset_growth: gross_profitability=0.50, asset_growth=0.20, reversal_1m=0.05, reversal_5d=0.25")
    ...
    order = {"baseline_quality": 0, "quality_with_asset_growth": 1}
    ...
    test_row = results_df.loc[results_df["Strategy"].eq("quality_with_asset_growth")]
    if not baseline_row.empty and not test_row.empty:
        base = baseline_row.iloc[0]
        comp = test_row.iloc[0]
        delta_df = pd.DataFrame(
            [
                {
                    "Strategy": "quality_with_asset_growth_vs_baseline",
    ...
    summary_path = run_dir / "quality_asset_growth_summary.csv"
    delta_path = run_dir / "quality_asset_growth_delta_vs_baseline.csv"
        ...
        json.dumps(
            {
                ...
                "notes": [
                    ...
                    "The existing asset_growth factor is used unchanged.",
                    "asset_growth receives aligned total_assets through script-level factor_params.",
