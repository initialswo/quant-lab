"""Focused comparison: lead 4-factor candidate with static vs dynamic regime-aware factor weights."""
...
def main() -> None:
    ...
    variants = [
        {"Variant": "Lead4F_Static", "dynamic_factor_weights": False},
        {"Variant": "Lead4F_Dynamic", "dynamic_factor_weights": True},
    ]
    ...
    for variant in variants:
        ...
        rows.append(
            {
                "Variant": vname,
                "DynamicFactorWeights": bool(variant["dynamic_factor_weights"]),
    ...
    print("LEAD 4FACTOR: STATIC VS DYNAMIC FACTOR WEIGHTS")
    print("-----------------------------------------------")
    print(disp.to_string(index=False))

    ts = datetime.utcnow().strftime("%Y%m%d_%H%M%S")
    outdir = Path("results") / f"dynamic_factor_weight_lead_comparison_{ts}"
    outdir.mkdir(parents=True, exist_ok=True)
    full_csv = outdir / "lead_dynamic_factor_weight_comparison.csv"
    sub_csv = outdir / "lead_dynamic_factor_weight_subperiods.csv"
    summary_json = outdir / "lead_dynamic_factor_weight_summary.json"
