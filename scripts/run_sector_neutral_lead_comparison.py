"""Focused comparison: lead 4-factor candidate with/without sector-neutral ranking."""
...
def main() -> None:
    ...
    variants = [
        {"Variant": "Lead4F_GlobalRank", "sector_neutral": False},
        {"Variant": "Lead4F_SectorNeutralRank", "sector_neutral": True},
    ]
    ...
    for variant in variants:
        ...
        cfg["sector_neutral"] = bool(variant["sector_neutral"])
        full = _run_one(cfg, run_cache=run_cache)
        vname = str(variant["Variant"])
        rows.append(
            {
                "Variant": vname,
                "SectorNeutral": bool(variant["sector_neutral"]),
    ...
    print("LEAD 4FACTOR: GLOBAL VS SECTOR-NEUTRAL RANKING")
    print("-----------------------------------------------")
    print(disp.to_string(index=False))

    ts = datetime.utcnow().strftime("%Y%m%d_%H%M%S")
    outdir = Path("results") / f"sector_neutral_lead_comparison_{ts}"
    outdir.mkdir(parents=True, exist_ok=True)
    full_csv = outdir / "lead_sector_neutral_comparison.csv"
    sub_csv = outdir / "lead_sector_neutral_subperiods.csv"
    summary_json = outdir / "lead_sector_neutral_summary.json"
